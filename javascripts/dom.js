const domString = (movieArray, config, whereToPrint, myCollectionMode = false) => {
  ...
  movieArray.forEach((movie, index) => {
    ...
    if (myCollectionMode) {
      strang += `<a class="btn deleteMovieFromCollectionEvent">&#10005;</a>`;
    }
    ...
    if (!myCollectionMode) {
      ...
      strang +=     `<p><a class="btn btn-primary updateMovieToWatched" role="button">Watched &#10003;</a></p>`;
    } else {
      strang += `<p>I'm going to put a star rating here one day.</p>`;
    }
    strang +=   `</div>`;
    strang +=  `</div>`;
    strang += `</div>`;
